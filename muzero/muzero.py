    ...
    def logging_loop(self, num_gpus):
        ...
        try:
            while info["training_step"] < self.config.training_steps:
                ...
                print(
                    f'L tst rwrd: {info["total_reward"]:.2f}. Trn step: {info["training_step"]}/{self.config.training_steps}. Plyd gms: {info["num_played_games"]}. Val ls: {info["value_loss"]}. Rwrd ls: {info["reward_loss"]}. Pol ls: {info["policy_loss"]}. Ls: {info["total_loss"]:.2f}',
                    end="\r",
                )
                counter += 1
                time.sleep(0.5)
